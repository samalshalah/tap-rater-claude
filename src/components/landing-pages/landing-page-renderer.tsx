...
export function DigitalBusinessCardTemplate({ page }: LandingPageRendererProps) {
  return (
    <LandingShell page={page} eyebrow="Digital business card">
      <ButtonList page={page} buttons={page.buttons} />
      <div className="mt-6 rounded-md border border-line bg-white p-4 text-sm leading-6 text-muted">
    ...
    <main className="min-h-screen bg-soft px-4 py-10">
      <section className="mx-auto max-w-xl">
        <div className="rounded-lg border border-line bg-white p-6 text-center shadow-sm md:p-8">
          {page.logoUrl ? <img src={page.logoUrl} alt="" className="mx-auto mb-5 h-16 w-16 rounded-md object-contain" /> : null}
          <p className="text-xs font-black uppercase tracking-[0.16em] text-brand">{eyebrow}</p>
          <h1 className="mt-3 text-3xl font-black leading-tight text-ink md:text-4xl">{page.headline}</h1>
...
function ButtonList({ page, buttons, compact = false }: { page: LandingPage; buttons: LandingPageButton[]; compact?: boolean }) {
  if (!buttons.length) {
    return <div className="rounded-md border border-line bg-white p-5 text-center text-sm text-muted shadow-sm">No buttons are configured for this page yet.</div>;
  }
    ...
    <div className={compact ? "mt-5 grid gap-3" : "grid gap-3"}>
      {buttons.map((button) => (
        <a
          ...
          className="block rounded-md bg-brand px-5 py-4 text-center text-sm font-black text-white shadow-sm transition hover:bg-ink"
        >
          {button.label}
...
function LandingForm({ page, defaultFields }: { page: LandingPage; defaultFields: string[] }) {
  ...
  return (
    <form onSubmit={submitForm} className="grid gap-3 rounded-md border border-line bg-white p-5 shadow-sm">
      {fields.map((field) => (
        <FormField key={field} field={field} />
      ))}
      <button disabled={status === "saving"} className="rounded-md bg-brand px-5 py-3 text-sm font-black text-white transition hover:bg-ink disabled:bg-muted">
...
function FormField({ field }: { field: string }) {
  ...
  if (isMessage) {
    return <textarea name={field} placeholder={label} className="min-h-28 rounded-md border border-line px-4 py-3 text-sm outline-none focus:border-brand" />;
  }

  return <input name={field} type={type} placeholder={label} className="rounded-md border border-line px-4 py-3 text-sm outline-none focus:border-brand" />;
}
